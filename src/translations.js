export const translations = {
    en: {
        story: {
            sectionTag: "Our Story",
            heading: "Two Souls, One Journey",

            paragraph1:
                "In the sacred land of devotion and blessings, two hearts found each other amidst faith, laughter, and divine grace.",

            paragraph2:
                "What began as a beautiful friendship slowly blossomed into a promise of forever — a journey woven with trust, compassion, and love.",
        },

        countdown: {
            sectionTag: "Countdown To Forever",
            heading: "The Celebration Begins In",

            days: "Days",
            hours: "Hours",
            minutes: "Minutes",
            seconds: "Seconds",
        },

        gallery: {
            sectionTag: "Cherished Moments",
            heading: "Gallery Of Memories",
        },

        events: {
            sectionTag: "Wedding Events",

            heading: "Celebration Timeline",

            items: [
                {
                    title: "Shri Ganesh Nimantran",
                    time: "22 June • 9:00 AM",
                    description:
                        "Seeking the blessings of Lord Ganesha and formally inviting Him to grace the wedding celebrations.",
                },

                {
                    title: "Sankadi Vinayak Poojan",
                    time: "25 June • 10:00 AM",
                    description:
                        "Traditional worship of Shri Sankadi Vinayak for auspiciousness and removal of all obstacles.",
                },

                {
                    title: "Haldi Ceremony",
                    time: "26 June • 9:00 AM",
                    description:
                        "A joyful ritual where turmeric is applied to bless the bride and groom with prosperity and happiness.",
                },

                {
                    title: "Mehendi Ceremony",
                    time: "26 June • 10:00 AM",
                    description:
                        "A colorful celebration adorned with beautiful henna designs, music, and family festivities.",
                },

                {
                    title: "Bhajan Sandhya",
                    time: "26 June • 5:00 PM",
                    description:
                        "An evening of devotional kirtans and bhajans filled with spiritual joy and divine remembrance.",
                },

                {
                    title: "Tilak Ceremony",
                    time: "27 June • 10:00 AM",
                    description:
                        "A sacred ritual marking the formal union of two families and the blessing of the couple.",
                },

                {
                    title: "Varmala Ceremony",
                    time: "27 June • 12:00 PM",
                    description:
                        "A beautiful and symbolic exchange of garlands between the bride and groom, marking the beginning of their sacred union amidst blessings, joy, and celebration.",
                },

                {
                    title: "Arrival of the Baraat",
                    time: "27 June • 4:00 PM",
                    description:
                        "The groom's grand wedding procession arrives amidst celebration, music, and festivity.",
                },

                {
                    title: "Paani Grahan Sanskar",
                    time: "27 June • 5:30 PM – 7:30 PM",
                    description:
                        "A traditional wedding ritual performed during the auspicious twilight hour.",
                },

                {
                    title: "Blessing Ceremony & Grand Feast",
                    time: "27 June • 7:00 PM",
                    description:
                        "Family and guests bless the newly married couple followed by a delightful celebratory feast.",
                },

            ],
        },

        scratch: {
            sectionTag: "Reveal The Sacred Date",
            heading: "Scratch To Reveal",

            description:
                "Gently scratch the divine seal to unveil the auspicious wedding date.",

            weddingDate: "Wedding Date",

            location: "Yashoda Nandan Dham, Vrindavan",

            revealMessage: "The celebration awaits you ✨",
        },

        rsvp: {
            heading: "RSVP",

            subheading:
                "We would be honored by your presence.",

            namePlaceholder: "Your Name",

            attending: "Joyfully Attending",

            notAttending:
                "Regretfully, Unable to Attend",

            guestPlaceholder: "Number of Guests",

            blessingPlaceholder:
                "Your Blessings/Message for the Couple",

            button: "Send RSVP ✨",

            thankYou: "Thank You ✨",

            thankYouMessage:
                "Your RSVP has been received.",
        },

        venue: {
            sectionTag: "Wedding Venue",

            detailsHeading: "Venue Details",

            venueName: "Yashoda Nandana Ashram",

            description:
                "A serene and spiritual venue in the divine land of Vrindavan, where the sacred wedding celebrations will take place.",

            city: "Vrindavan, Uttar Pradesh",

            venueType: "Wedding Venue",

            directions: "Get Directions ✨",
        },
    },

    hi: {
        story: {
            sectionTag: "हमारी कहानी",

            heading: "दो आत्माएँ, एक यात्रा",

            paragraph1:
                "भक्ति और आशीर्वाद की पावन भूमि में, दो दिलों ने श्रद्धा, हँसी और दिव्य कृपा के बीच एक-दूसरे को पाया।",

            paragraph2:
                "जो एक सुंदर मित्रता के रूप में शुरू हुआ, वह धीरे-धीरे जीवनभर के साथ के वचन में बदल गया — विश्वास, करुणा और प्रेम से बुनी एक यात्रा।",
        },

        countdown: {
            sectionTag: "सदा के साथ की प्रतीक्षा",

            heading: "उत्सव प्रारम्भ होने में",

            days: "दिन",
            hours: "घंटे",
            minutes: "मिनट",
            seconds: "सेकंड",
        },

        gallery: {
            sectionTag: "अनमोल यादें",

            heading: "स्मृतियों की झलकियाँ",
        },

        events: {
            sectionTag: "विवाह समारोह",

            heading: "उत्सव की समय-सारिणी",

            items: [
                {
                    title: "श्री गणेश निमंत्रण",
                    time: "22 जून • प्रातः 9:00 बजे",
                    description:
                        "विवाहोत्सव के शुभारम्भ हेतु श्री गणेश जी को आमंत्रित कर उनका मंगल आशीर्वाद प्राप्त किया जाएगा।",
                },

                {
                    title: "संकड़ी विनायक पूजन",
                    time: "25 जून • प्रातः 10:00 बजे",
                    description:
                        "सभी विघ्नों के निवारण एवं मंगल कार्य की सिद्धि हेतु श्री संकड़ी विनायक का पूजन।",
                },

                {
                    title: "हल्दी समारोह",
                    time: "26 जून • प्रातः 9:00 बजे",
                    description:
                        "दूल्हा-दुल्हन को शुभता, सौभाग्य एवं आनंद का आशीर्वाद प्रदान करने वाला पारंपरिक संस्कार।",
                },

                {
                    title: "मेहंदी समारोह",
                    time: "26 जून • प्रातः 10:00 बजे",
                    description:
                        "रंग, संगीत, उल्लास एवं सुंदर मेहंदी अलंकरणों से सुसज्जित आनंदमय समारोह।",
                },

                {
                    title: "भजन संध्या",
                    time: "26 जून • सायं 5:00 बजे",
                    description:
                        "भक्ति, कीर्तन एवं श्रीहरि स्मरण से परिपूर्ण दिव्य संगीतमय संध्या।",
                },

                {
                    title: "तिलक समारोह",
                    time: "27 जून • प्रातः 10:00 बजे",
                    description:
                        "दो परिवारों के शुभ मिलन एवं वर-वधू के मंगल बंधन का पावन संस्कार।",
                },

                {
                    title: "वरमाला समारोह",
                    time: "27 जून • दोपहर 12:00 बजे",
                    description:
                        "वर एवं वधू द्वारा शुभ वरमाला का आदान-प्रदान, जो उनके पावन मिलन, प्रेम और आजीवन साथ के मंगल संकल्प का प्रतीक है।",
                },

                {
                    title: "बारात आगमन",
                    time: "27 जून • सायं 4:00 बजे",
                    description:
                        "हर्षोल्लास, संगीत एवं उत्सव के मध्य वर पक्ष की भव्य बारात का आगमन।",
                },

                {
                    title: "पाणिग्रहण संस्कार",
                    time: "27 जून • सायं 5:30 – 7:30 बजे",
                    description:
                        "शुभ गोधूलि बेला में सम्पन्न होने वाला वैदिक वैवाहिक संस्कार।",
                },

                {
                    title: "आशीर्वाद समारोह एवं प्रीतिभोज",
                    time: "27 जून • सायं 7:00 बजे",
                    description:
                        "नवदंपति को आशीर्वाद प्रदान करने के उपरांत स्नेहपूर्ण प्रीतिभोज का आयोजन।",
                },

            ],
        },

        scratch: {
            sectionTag: "शुभ तिथि जानें",

            heading: "खुरच कर देखें",

            description:
                "दिव्य मोहर को धीरे से खुरचें और विवाह की शुभ तिथि देखें।",

            weddingDate: "विवाह तिथि",

            location: "यशोदा नन्दन धाम, वृन्दावन",

            revealMessage:
                "उत्सव आपका इंतज़ार कर रहा है ✨",
        },

        rsvp: {
            heading: "उपस्थिति पुष्टि",

            subheading:
                "आपकी उपस्थिति हमारे लिए सौभाग्य होगी।",

            namePlaceholder: "आपका नाम",

            attending: "सहर्ष उपस्थित होंगे",

            notAttending:
                "क्षमा करें, उपस्थित नहीं हो पाएँगे",

            guestPlaceholder: "अतिथियों की संख्या",

            blessingPlaceholder:
                "दूल्हा-दुल्हन के लिए संदेश / आशीर्वाद",

            button: "RSVP भेजें ✨",

            thankYou: "धन्यवाद ✨",

            thankYouMessage:
                "आपकी उपस्थिति पुष्टि प्राप्त हो गई है।",
        },

        venue: {
            sectionTag: "विवाह स्थल",

            detailsHeading: "स्थल विवरण",

            venueName: "यशोदा नंदन आश्रम",

            description:
                "वृन्दावन की पावन भूमि में स्थित एक शांत एवं आध्यात्मिक स्थल, जहाँ विवाह समारोह सम्पन्न होगा।",

            city: "वृन्दावन, उत्तर प्रदेश",

            venueType: "विवाह स्थल",

            directions: "मार्ग देखें ✨",
        },
    },
};